...
# --- Nová globální proměnná na skóre ---
score = 0
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            if self.placed and DISPENSER_RECT.collidepoint(self.rect.center):
                if assembled_items:
                    ...
                    # vytvoř recepturu z položek
                    recipe = {"cone": None, "scoops": [], "topping": None}

                    for item in assembled_items:
                        if item.image == cone_item.image:
                            recipe["cone"] = "klasický"  # (zatím jen jeden typ – nastav natvrdo nebo podle assetu)
                        elif item.image == scoop1_item.image:
                            recipe["scoops"].append("čokoláda")
                        elif item.image == scoop2_item.image:
                            recipe["scoops"].append("šmoulová")
                        # Pokud máš obrázek polevy, přidej kontrolu zde

                    # Vytvoř nový IceCream sprite
                    assembled_icecream = IceCream(assembled_items.copy(), recipe)
                    icecream_group.add(assembled_icecream)
                    assembled_items.clear()  # vyprázdni složenou zmrzlinu
                    
                    # Kontrola kolize
                    if pygame.sprite.spritecollideany(assembled_icecream, dispenser_group):
                        if customer.order and assembled_icecream.matches(customer.order):
...
# --- Třída pro zmrzlinu (složenou) jako sprite ---
class IceCream(pygame.sprite.Sprite):
    def __init__(self, items, recipe):
        ...
        self.recipe = recipe  # {'cone': ..., 'scoops': [...], 'topping': ...}

        # vykreslení vrstvených obrázků
    ...
    def check_with_order(self, order):
        if not order:
            return False
        cone_ok = self.recipe['cone'] == order.cone
        scoops_ok = sorted(self.recipe['scoops']) == sorted(order.scoops)
        topping_ok = self.recipe['topping'] == order.topping
        return cone_ok and scoops_ok and topping_ok      


# --- Třída pro výdejní místo jako sprite ---
class Dispenser(pygame.sprite.Sprite):
    def __init__(self):
        super().__init__()
        self.image = pygame.Surface((160, 200), pygame.SRCALPHA)
        self.image.fill((220, 255, 220))
        pygame.draw.rect(self.image, (0, 120, 0), self.image.get_rect(), 2)
        self.rect = self.image.get_rect(topleft=(WIDTH - 180, HEIGHT - 220))

    def check_delivery(self, icecream_sprite, order):
        global score
        if self.rect.colliderect(icecream_sprite.rect):
            if icecream_sprite.check_with_order(order):
                print("Objednavka OK!")
                score += 1
                player.deliver_to(customer)
                return True
            else:
                print("Objednavka NESEDI!")
                return False
        return False       
...
while running:
    ...
    if STATE == "intro":
        ...
        # Vykresli dispenser sprite
        dispenser_group.draw(screen)

        # Vykresli zmrzlinu (icecream)
        icecream_group.draw(screen)

        # Vykresli skóre
        score_text = pygame.font.SysFont("arial", 24).render(f"Skóre: {score}", True, BLACK)
